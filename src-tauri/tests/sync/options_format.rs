use noriskclient_launcher_v3_lib::sync::model::MergeFormat;
use noriskclient_launcher_v3_lib::sync::options_format::*;
...
fn json_style_values_survive() {
    ...
    let doc = OptionsDocument::parse(text, MC);
    assert_eq!(
        doc.get("resourcePacks"),
        Some("[\"vanilla\",\"file/foo.zip\"]")
    );
    assert_eq!(doc.render(), text);
}
