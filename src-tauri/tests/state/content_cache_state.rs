use noriskclient_launcher_v3_lib::state::content_cache_state::*;
use noriskclient_launcher_v3_lib::state::db;
use noriskclient_launcher_v3_lib::state::profile_state::ModPackSource;
use sqlx::Row;
...
async fn stale_rows_are_still_served() {
    let m = manager().await;
    m.put_entry(K, "old", None, Some("value".to_string()), 0)
        .await;
    tokio::time::sleep(std::time::Duration::from_millis(5)).await;

    let entry = m
        .get_entry::<String>(K, "old")
        .await
        .expect("stale row must be returned");
...
async fn alias_can_be_reassigned() {
    ...
    let by_alias = m
        .get_entry::<String>(K, "shared-slug")
        .await
...
async fn undeserializable_row_is_a_miss_and_gets_deleted() {
    ...
    sqlx::query(
        "INSERT INTO cache (id, data_type, data, expires) VALUES ('bad', ?1, 'not json', ?2)",
    )
    .bind(K)
    .bind(i64::MAX)
    .execute(&pool)
...
async fn expiry_sweep_spares_file_hashes_and_the_grace_window() {
    let m = manager().await;
    m.put_entry(K, "fresh", None, Some("x".to_string()), TTL_METADATA_MS)
        .await;
    m.put_entry(K, "recently-expired", None, Some("x".to_string()), 0)
        .await;
    m.put_file_hashes(vec![(
        "/some/path".to_string(),
        FileHashEntry {
...
async fn key_formats_are_stable() {
    ...
    assert_eq!(
        project_versions_cache_key("sodium", &None, &None),
        "sodium::"
    );
...
async fn no_pool_means_miss_not_panic() {
    ...
    m.put_entry(K, "a", None, Some("x".to_string()), TTL_METADATA_MS)
        .await;
...
async fn clear_removes_every_row() {
    ...
    assert_eq!(
        stats.rows_deleted, 51,
        "file hashes go too — clear means clear"
    );
