...
use noriskclient_launcher_v3_lib::state::profile_state::{ModLoader, NoriskModIdentifier, Profile};
use noriskclient_launcher_v3_lib::state::profile_store::*;
use sqlx::Row;
use std::collections::{HashMap, HashSet};
use uuid::Uuid;

async fn store() -> ProfileStore {
    let handle = crate::harness::temp_db_handle().await;
...
async fn the_same_mod_id_may_live_in_two_profiles() {
    ...
    store
        .upsert_many(&[first.clone(), second.clone()])
        .await
        .unwrap();
    let loaded = store.load_all().await.unwrap();
...
async fn a_store_without_a_database_refuses_to_store_profiles() {
    ...
    assert!(
        store.load_all().await.is_err(),
        "reading must not look empty"
    );
...
async fn the_lookup_columns_agree_with_the_stored_source() {
    ...
    for row in rows {
        let source: noriskclient_launcher_v3_lib::state::profile_state::ModSource =
...
async fn a_malformed_entry_is_quarantined_not_dropped() {
    ...
    let pool = store.pool().await.unwrap();
    let kept: String =
        sqlx::query_scalar("SELECT raw FROM profiles_legacy_import WHERE parsed = 0")
            .fetch_one(&pool)
            .await
            .unwrap();
...
async fn a_duplicate_id_keeps_the_last_and_escrows_the_first() {
    ...
    assert_eq!(
        outcome.unparsed, 1,
        "the loser must be recorded, not silently gone"
    );
...
async fn a_targeted_write_never_reaches_another_profile() {
    ...
    store
        .set_mods_enabled(second.id, &[shared], false)
        .await
        .unwrap();
...
async fn a_targeted_write_past_the_sql_variable_limit_still_lands() {
    let store = store().await;
    let mut profile = corpus().into_iter().next().unwrap();
    let template = profile.mods.first().cloned().unwrap_or_else(|| {
        corpus()
            .into_iter()
            .find_map(|p| p.mods.first().cloned())
            .unwrap()
    });
    ...
    let changed = store
        .set_mods_enabled(profile.id, &ids, false)
        .await
        .unwrap();
...
async fn upserting_one_mod_leaves_its_neighbours_alone() {
    ...
    store
        .upsert_mods(profile.id, &[(1, changed.clone())])
        .await
        .unwrap();
...
async fn upserting_an_unknown_mod_appends_it_without_touching_the_rest() {
    ...
    store
        .upsert_mods(profile.id, &[(at, fresh.clone())])
        .await
        .unwrap();
    ...
    assert_eq!(
        loaded.mods.last().unwrap().id,
        fresh.id,
        "a new mod lands at its ordinal"
    );
}
...
async fn a_targeted_mod_write_never_reaches_another_profile() {
    ...
    let loaded = store.load_all().await.unwrap();
    assert_eq!(
        loaded[&second.id].mods[0].display_name, second.mods[0].display_name,
...
fn fixture_files() -> Vec<std::path::PathBuf> {
    ...
    let mut files: Vec<std::path::PathBuf> = std::fs::read_dir(&path)
        ...
        .filter(|p| {
            ...
            !name.ends_with(".backup.meta") && (is_profiles_json(name) || name.ends_with(".backup"))
        })
        .collect();
    files.sort();
    files
...
async fn real_profiles_migrate_through_the_production_import() {
    ...
    crate::harness::open_at_for_test(&reopened, &path).await;
...
async fn a_failing_verification_rolls_the_whole_import_back() {
    ...
    assert_eq!(
        profiles, 1,
        "the good entry lands, the bad one is quarantined"
    );
}
...
async fn every_source_entry_is_escrowed() {
    ...
    assert_eq!(
        escrowed, 23,
        "the escrow must hold every entry, not just the good ones"
    );
}
...
async fn real_profiles_read_through_cost() {
    ...
    println!(
        "  load_all: {} profiles in {}ms",
        map.len(),
        started.elapsed().as_millis()
    );

    let mut by_mods: Vec<(usize, Uuid)> = map.iter().map(|(id, p)| (p.mods.len(), *id)).collect();
    by_mods.sort();
    ...
    async fn read_one(pool: &sqlx::SqlitePool, id: Uuid) -> Profile {
            ...
            .unwrap()
        {
            profile
                .disabled_norisk_mods_detailed
                .insert(row_to_disabled(&row).unwrap().1);
        }
        profile
    }
    ...
    println!(
        "  list_profiles today (map clone of all):      {}ms",
    ...
    println!(
        "  list_profiles today + JSON to frontend:      {}ms",
        started.elapsed().as_millis() / 5
    );
    ...
    println!(
        "  list_profiles under read-through (load_all): {}ms",
        started.elapsed().as_millis() / 5
    );
    ...
    println!(
        "  what the UI really does today (clone all, strip mods, JSON): {}ms",
        started.elapsed().as_millis() / 5
    );
    ...
    println!(
        "  new slim list path (no mod clone) + JSON:     {}ms",
        started.elapsed().as_millis() / 5
    );

    let started = std::time::Instant::now();
    for _ in 0..5 {
        let rows = sqlx::query("SELECT * FROM profiles")
            .fetch_all(&pool)
            .await
            .unwrap();
    ...
    println!(
        "  same list straight from SQL, mods never read: {}ms",
        started.elapsed().as_millis() / 5
    );
    ...
    for (label, (mods, id)) in [("median", median), ("biggest", biggest)] {
        ...
        for i in 0..10 {
            ...
            store
                .upsert_many(std::slice::from_ref(&profile))
                .await
                .unwrap();
        }
        println!(
...
async fn export_profiles_to_json() {
    ...
    let handle = db::new_handle();
    crate::harness::open_at_for_test(&handle, std::path::Path::new(&source)).await;
    let store = ProfileStore::new(handle);
