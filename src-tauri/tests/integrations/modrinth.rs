use noriskclient_launcher_v3_lib::integrations::modrinth::*;
...
fn rejects_links_from_elsewhere() {
    assert_eq!(
        ids_from_cdn_url("https://example.com/data/a/versions/b/c.jar"),
        None
    );
    assert_eq!(
        ids_from_cdn_url("https://cdn.modrinth.com/other/a/versions/b/c.jar"),
        None
    );
    assert_eq!(
        ids_from_cdn_url("https://cdn.modrinth.com/data/a/files/b/c.jar"),
        None
    );
}
...
fn rejects_a_truncated_link() {
    assert_eq!(
        ids_from_cdn_url("https://cdn.modrinth.com/data/4XJZeZbM"),
        None
    );
    assert_eq!(
        ids_from_cdn_url("https://cdn.modrinth.com/data/4XJZeZbM/versions"),
        None
    );
...
fn accepts_the_hosts_modrinth_allows() {
    ...
    assert!(is_whitelisted_modpack_url(
        "https://github.com/o/r/releases/x.jar"
    ));
    assert!(is_whitelisted_modpack_url(
        "https://raw.githubusercontent.com/o/r/main/x.jar"
    ));
    assert!(is_whitelisted_modpack_url(
        "https://gitlab.com/o/r/-/raw/main/x.jar"
    ));
}
