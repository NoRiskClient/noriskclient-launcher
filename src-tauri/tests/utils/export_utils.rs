...
use noriskclient_launcher_v3_lib::utils::export_utils::*;
use std::path::Path;
use tokio::fs;
...
async fn no_selection_exports_no_files() {
    let dir = tempfile::tempdir().unwrap();
    fs::write(dir.path().join("options.txt"), "x")
        .await
        .unwrap();
...
async fn linked_in_content_is_still_collected() {
    ...
    collect_all_files_recursive(&instance, &mut files)
        .await
        .unwrap();
...
fn loader_versions_lose_the_game_version_maven_prefix() {
    assert_eq!(
        normalize_loader_version("1.21.1-52.1.0", "1.21.1"),
        "52.1.0"
    );
    assert_eq!(
...
fn relative_zip_path_normalises_separators_and_rejects_outsiders() {
    ...
    assert_eq!(
        relative_zip_path(base, Path::new("/instances/bar/a.json")),
        None
    );
}
