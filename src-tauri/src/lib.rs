...
pub fn run() {
    // Android app processes have no HOME/XDG environment, which breaks
    // directories::ProjectDirs (LAUNCHER_DIRECTORY). Point everything at the
    // app-internal storage before any path is resolved.
    #[cfg(target_os = "android")]
    {
    ...
    // Own the tokio runtime explicitly (instead of #[tokio::main]) so the same
    // entry point works for both the desktop binary and the mobile library.
    ...
    // Keep the runtime context entered so `tokio::spawn` from event callbacks works.
    ...
    builder
        .setup(|app| {
            ...
            tauri::async_runtime::spawn(async move {
                ...
                #[cfg(mobile)]
                {
                    info!("Mobile: main window visibility is managed by the OS activity, skipping show().");
                }
                #[cfg(desktop)]
