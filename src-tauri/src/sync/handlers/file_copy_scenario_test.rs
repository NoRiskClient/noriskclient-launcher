...
// A shared servers.dat is the everyday case for file_copy: one list of servers, several profiles.
// These tests say what the player expects to see, not how the handler reaches it.
...
async fn a_profile_that_never_had_the_file_gets_it() {
    ...
    assert_eq!(read(&alice.file(TARGET)).await.as_deref(), Some("from the pack"));
}
...
async fn a_change_someone_else_made_later_is_not_quietly_overwritten() {
    ...
    assert_eq!(read(&s.master).await.as_deref(), Some("bob got there first"));
    assert!(
