...
static NEO_FORGE_MAVEN_REPO_BASE: &str =
...
impl NeoForgeProvider {
    ...
    pub async fn download_installer_jar<F>(
        ...
        F: Fn(u64, u64),
    {
        ...
        let url = format!(
            "{}/{loader}/neoforge-{loader}-installer.jar",
            NEO_FORGE_MAVEN_REPO_BASE,
            loader = custom_server.loader_version.clone().unwrap_or_default()
        );
        let content = download_file(&url, on_progress).await?;
        let _ = fs::write(
            path.join(format!(
                "neoforge-{}.jar",
                custom_server.loader_version.clone().unwrap_or_default()
            )),
            content,
        )
        .await;
