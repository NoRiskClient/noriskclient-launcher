...
impl FoliaProvider {
    ...
    pub async fn download_server_jar<F>(custom_server: &CustomServer, on_progress: F) -> Result<()> where F : Fn(u64, u64) {
        ...
        let url = format!("{}/versions/{}/builds/{}/downloads/{}", FOLIA_API_BASE, &custom_server.mc_version, &custom_server.loader_version, format!("server-{}-{}.jar", custom_server.mc_version, latest_build.build));
