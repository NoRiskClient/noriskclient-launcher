...
use crate::{
    app::{api::ApiEndpoints, app_data::LauncherOptions, gui::get_options},
    custom_servers::{
        models::{CustomServerEventPayload, CustomServerTokenResponse},
        providers::forwarding_manager::ForwardingManagerProvider,
    },
    minecraft::{
        java::{find_java_binary, jre_downloader, JavaRuntime},
...
use super::{
    models::{
        CustomServer, CustomServerProgressEventPayload, CustomServerType, LatestRunningServer,
    },
    providers::{forge::ForgeProvider, vanilla::VanillaProvider},
};
...
impl CustomServerManager {
    pub async fn initialize_server(
        ...
        match server.r#type {
            CustomServerType::VANILLA => {
                ...
                let _ = VanillaProvider::download_server_jar(
                    &server,
    ...
    async fn create_eula_file(custom_server: &CustomServer) -> Result<()> {
        ...
        let _ = fs::write(path.join("eula.txt"), Vec::from(content)).await;
        Ok(())
    }
    ...
    pub async fn run_server(
        ...
        let custom_server_path = LAUNCHER_DIRECTORY
            .data_dir()
            .join("custom_servers")
            .join(&custom_server.id);
        ...
        let mut running_task = java_runtime
            .run_server(2048, 2048, &custom_server_path)
            .await?;

        Self::store_latest_running_server(None, running_task.id(), Some(custom_server.id.clone()))
            .await?;
    ...
    pub async fn read_and_process_server_log_file(
        window: &Arc<Mutex<Window>>,
        server_id: &str,
    ) -> Result<()> {
        let path = LAUNCHER_DIRECTORY
    ...
    async fn handle_property_file_defaults(server_id: &str) {
        ...
        let mut force_defauls: Vec<(String, String)> = vec![
            ("enable-rcon".to_owned(), "true".to_owned()),
            ("rcon.port".to_owned(), "25594".to_owned()),
            ("rcon.password".to_owned(), "minecraft".to_owned()),
            ("max-players".to_owned(), "10".to_owned()), // TODO: Add slider support
            ("broadcast-rcon-to-ops".to_owned(), "false".to_owned()),
        ];
    ...
    pub async fn store_latest_running_server(
        forwarder_process_id: Option<u32>,
