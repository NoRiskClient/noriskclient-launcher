...
// Nothing listens on port 1, so any real request fails instantly. Ok() therefore
// proves the network was never needed; Err() proves it was attempted.
...
async fn cached_file_with_matching_sha1_skips_network() {
    ...
    assert!(result.is_ok(), "cached file must launch offline: {result:?}");
}
...
async fn cached_file_with_wrong_sha1_hits_network() {
    ...
    assert!(result.is_err(), "corrupt cache must not be silently accepted");
}
...
async fn force_overwrite_ignores_cache() {
    ...
    assert!(result.is_err(), "force_overwrite must always hit the network");
}
