...
use super::{
    ...
    app_data::{LauncherOptions, LauncherProfiles},
    modrinth_api::{
...
#[allow(dead_code)] // Seems like we might need this later
...
fn open_url(url: &str, handle: tauri::AppHandle) -> Result<(), String> {
    ...
    // TODO: these errors are not being handled
    ...
    match ApiEndpoints::norisk_featured_mods(branch).await {
        Ok(result) => {
            ...
            for mod_id in result {
                if let Ok(mod_info) = ModrinthApiEndpoints::get_mod_info(&mod_id).await {
    ...
    match ApiEndpoints::norisk_featured_resourcepacks(branch).await {
        Ok(result) => {
            ...
            for resourcepack_id in result {
                if let Ok(resourcepack_info) =
                    ModrinthApiEndpoints::get_resourcepack_info(&resourcepack_id).await
                {
    ...
    match ApiEndpoints::norisk_featured_shaders(branch).await {
        Ok(result) => {
            ...
            for shader_id in result {
                if let Ok(shader_info) = ModrinthApiEndpoints::get_shader_info(&shader_id).await {
    ...
    match ApiEndpoints::norisk_featured_datapacks(branch).await {
        Ok(result) => {
            ...
            for datapack_id in result {
                if let Ok(datapack_info) =
                    ModrinthApiEndpoints::get_datapack_info(&datapack_id).await
                {
...
async fn install_mod_and_dependencies(
    ...
    ModrinthApiEndpoints::install_mod_and_dependencies(slug, params, &required_mods)
        .await
...
async fn download_template_and_open_explorer() -> Result<(), String> {
    ...
    let user_dirs = UserDirs::new()
        .context("Error getting user directories")
        .map_err(|e| e.to_string())?;
    let downloads_dir = user_dirs
        .download_dir()
        .context("Error getting download directory")
        .map_err(|e| e.to_string())?;
    ...
    CapeApiEndpoints::show_in_folder(
        downloads_dir
            ...
            .into_os_string()
            .to_str()
            .context("Error converting path to string")
            .map_err(|e| e.to_string())?,
    )
    .map_err(|e| e.to_string())?;
...
pub async fn minecraft_auth_update_norisk_token(
    ...
    let mut accounts = minecraft_auth_get_store().await?;
    accounts.refresh_norisk_token(&credentials).await
}
...
#[tauri::command]
async fn get_custom_mods_folder(
    options: LauncherOptions,
    branch: &str,
    mc_version: &str,
...
async fn get_launch_manifest(branch: &str) -> Result<NoRiskLaunchManifest, crate::error::Error> {
    ApiEndpoints::launch_manifest(branch).await
}
