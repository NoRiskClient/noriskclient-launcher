...
pub async fn copy_mc_dir_all(src: &PathBuf, dst: &PathBuf, app: &tauri::AppHandle) -> Result<(), String> {
...
async fn copy_mc_data(path: &str, branch: &str, app: tauri::AppHandle) -> Result<(), String> {
    ...
    if resource_packs_path.exists() {
        copy_mc_dir_all(&resource_packs_path, &new_resource_packs_path, &app).await.map_err(|e| format!("unable to copy resource packs: {:?}", e))?;
    }
    ...
    if shader_packs_path.exists() {
        copy_mc_dir_all(&shader_packs_path, &new_shader_packs_path, &app).await.map_err(|e| format!("unable to copy shader packs: {:?}", e))?;
    }
    ...
    if saves_path.exists() {
        copy_mc_dir_all(&saves_path, &new_saves_path, &app).await.map_err(|e| format!("unable to copy saves: {:?}", e))?;
    }
...
async fn copy_branch_data(old_branch: &str, new_branch: &str, app: tauri::AppHandle) -> Result<(), String> {
    ...
    if old_resource_packs_path.exists() {
        copy_mc_dir_all(&old_resource_packs_path, &new_resource_packs_path, &app).await.map_err(|e| format!("unable to copy resource packs: {:?}", e))?;
    }
    ...
    if old_shader_packs_path.exists() {
        copy_mc_dir_all(&old_shader_packs_path, &new_shader_packs_path, &app).await.map_err(|e| format!("unable to copy shader packs: {:?}", e))?;
    }
    ...
    if old_saves_path.exists() {
        copy_mc_dir_all(&old_saves_path, &new_saves_path, &app).await.map_err(|e| format!("unable to copy saves: {:?}", e))?;
    }
    ...
    if old_nrc_data_path.exists() {
        copy_mc_dir_all(&old_nrc_data_path, &new_nrc_data_path, &app).await.map_err(|e| format!("unable to copy NoRiskClient data: {:?}", e))?;
    }
