...
use crate::{
    ...
    app::{
        ...
        gui::FileData,
        modrinth_api::{Datapack, DatapackInfo, ModrinthApiEndpoints},
    },
...
impl DataPackManager {
    ...
    pub async fn get_featured_datapacks(
        branch: &str,
        mc_version: &str,
        ...
        for datapack_id in featured {
            ...
            let mut datapack_info =
                ModrinthApiEndpoints::get_project::<DatapackInfo>(&datapack_id).await?;
    ...
    pub async fn download_datapack(
        ...
        let datapack_path =
            Self::get_datapack_folder(&options, branch, world).join(&datapack.file_name);
        ...
        if !datapack_path.exists() || datapack_path.is_dir() {
            ...
            if let Some(url) = &datapack.url {
                ...
                let retrieved_bytes = download_file(url, |a, b| {
                    window
                        .emit(
                            "addons-progress",
                            AddonsProgress {
                                identifier: datapack.slug.clone(),
                                current: a,
                                max: b,
                            },
                        )
                ...
                info!(
                    ...
                    &datapack.file_name, &datapack.world_name
                );
            }
        }
    ...
    pub async fn get_custom_datapack_filenames(
        options: LauncherOptions,
        branch: &str,
        world: &str,
    ...
    pub async fn save_custom_datapack_to_folder(
        options: LauncherOptions,
        branch: &str,
        world: &str,
        file: FileData,
    ...
    pub async fn delete_datapack_file(
        options: LauncherOptions,
        branch: &str,
        world: &str,
        file_name: &str,
