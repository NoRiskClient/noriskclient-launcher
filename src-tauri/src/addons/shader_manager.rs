...
use crate::{
    ...
    app::{
        ...
        gui::FileData,
        modrinth_api::{ModrinthApiEndpoints, Shader, ShaderInfo},
    },
...
impl ShaderManager {
    pub async fn get_shader(slug: &str, params: &str) -> Result<Shader, Error> {
        ...
        let project_version = shader_versions
            .first()
            .ok_or(Error::from(ErrorKind::OtherError(
                "No project version found".to_string(),
            )))?;
    ...
    pub async fn get_featured_shaders(
        branch: &str,
        mc_version: &str,
        ...
        for shader_id in featured {
            ...
            let mut shader_info =
                ModrinthApiEndpoints::get_project::<ShaderInfo>(&shader_id).await?;
            {
    ...
    pub async fn get_custom_shaders_filenames(
    ...
    pub async fn save_custom_shader_to_folder(
        options: LauncherOptions,
        branch: &str,
        file: FileData,
    ) -> Result<(), Error> {
    ...
    pub async fn delete_shader_file(
        options: LauncherOptions,
        branch: &str,
        file_name: &str,
    ) -> Result<(), Error> {
        ...
        let settings_file = shaders_folder.join(format!("{file_name}.txt"));
