...
use crate::{
    addons::progress::AddonsProgress,
    app::{
        api::ApiEndpoints,
        app_data::LauncherOptions,
        gui::FileData,
        modrinth_api::{ModrinthApiEndpoints, ResourcePack, ResourcePackInfo},
    },
...
impl ResourcePackManager {
    ...
    pub async fn get_featured_resourcepacks(
        branch: &str,
        mc_version: &str,
        ...
        for resourcepack_id in featured {
            let project_members =
                ModrinthApiEndpoints::get_project_members(&resourcepack_id).await?;
            let mut resourcepack_info =
                ModrinthApiEndpoints::get_project::<ResourcePackInfo>(&resourcepack_id).await?;
    ...
    pub async fn download_resourcepack(
        ...
        let resourcepack_path =
            Self::get_resourcepack_folder(&options, branch).join(&resourcepack.file_name);
                ...
                let retrieved_bytes = download_file(url, |a, b| {
                    window
                        .emit(
                            "addons-progress",
                            AddonsProgress {
                                identifier: resourcepack.slug.clone(),
                                current: a,
                                max: b,
                            },
                        )
    ...
    pub async fn get_custom_resourcepack_filenames(
        options: LauncherOptions,
        branch: &str,
        ...
        while let Some(entry) = resourcepacks_read.next_entry().await? {
            if entry.file_type().await?.is_file()
                && std::path::Path::new(entry.file_name().to_str().unwrap())
                    .extension()
    ...
    pub async fn save_custom_resourcepack_to_folder(
        options: LauncherOptions,
        branch: &str,
        file: FileData,
    ) -> Result<(), Error> {
    ...
    pub async fn delete_resourcepack_file(
        options: LauncherOptions,
        branch: &str,
        file_name: &str,
    ) -> Result<(), Error> {
