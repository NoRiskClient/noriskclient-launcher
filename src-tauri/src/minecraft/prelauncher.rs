...
use crate::app::api::{LoaderMod, LoaderSubsystem, ModSource, NoRiskLaunchManifest};
...
pub async fn retrieve_and_copy_mods(
    ...
    for (mod_idx, current_mod) in mods.iter().enumerate() {
        ...
        if (!current_mod.required && !current_mod.enabled)
            || additional_mods.iter().any(|m| {
                m.source.get_slug() == current_mod.source.get_slug()
                    && m.source.get_repository() == "PLACEHOLDER"
            })
        {
            continue;
        }
        ...
        if installed_mods
            .iter()
            .any(|loader_mod| loader_mod.is_same_slug(current_mod))
        {
                ...
                ModSource::Repository {
                    ...
                    let retrieved_bytes = download_file(&download_url, |a, b| {
                        progress.progress_update(ProgressUpdate::set_for_step(
                            ProgressUpdateSteps::DownloadNoRiskClientMods,
        ...
        fs::copy(
            &current_mod_path,
            mods_path.join(format!("{}.jar", current_mod.name.replace(".jar", ""))),
        )
        .await?;
...
pub async fn retrieve_shaders(
    data: &Path,
    manifest: &NoRiskLaunchManifest,
    shaders: &[Shader],
    progress: &impl ProgressReceiver,
) -> Result<()> {
    let shader_path = data
        .join("gameDir")
        .join(&manifest.build.branch)
        .join("shaderpacks");
    ...
    for (shader_idx, current_shader) in shaders.iter().enumerate() {
        if installed_shaders
            .iter()
            .any(|shader| shader.slug == current_shader.slug)
        {
        ...
        if !current_shader_path.exists() {
            ...
            if let Some(url) = &current_shader.url {
                info!(
                    "downloading shader {} from {}",
                    &current_shader.file_name, url
                );

                let retrieved_bytes = download_file(url, |a, b| {
                    progress.progress_update(ProgressUpdate::set_for_step(
                        ProgressUpdateSteps::DownloadShader,
                        get_progress(shader_idx, a, b),
                        max,
                    ));
                })
                .await?;
...
pub async fn retrieve_resourcepacks(
    data: &Path,
    manifest: &NoRiskLaunchManifest,
    resourcepacks: &[ResourcePack],
    progress: &impl ProgressReceiver,
) -> Result<()> {
    let resourcepack_path = data
        .join("gameDir")
        .join(&manifest.build.branch)
        .join("resourcepacks");
    ...
    for (resourcepack_idx, current_resourcepack) in resourcepacks.iter().enumerate() {
        if installed_resourcepacks
            .iter()
            .any(|resourcepack| resourcepack.slug == current_resourcepack.slug)
        {
        ...
        progress.progress_update(ProgressUpdate::set_label(format!(
            "Downloading resourcepack {}",
            &current_resourcepack.title
        )));
        ...
        if !current_resourcepack_path.exists() {
            ...
            if let Some(url) = &current_resourcepack.url {
                info!(
                    "downloading resourcepack {} from {}",
                    &current_resourcepack.file_name, url
                );

                let retrieved_bytes = download_file(url, |a, b| {
                    progress.progress_update(ProgressUpdate::set_for_step(
                        ProgressUpdateSteps::DownloadResourcePack,
                        get_progress(resourcepack_idx, a, b),
                        max,
                    ));
                })
                .await?;
...
pub async fn retrieve_datapacks(
    data: &Path,
    manifest: &NoRiskLaunchManifest,
    datapacks: &[Datapack],
    progress: &impl ProgressReceiver,
    ...
    for (datapack_idx, current_datapack) in datapacks.iter().enumerate() {
        let datapack_path = saves_path
            .join(current_datapack.world_name.clone())
            .join("datapacks");
        ...
        if installed_datapacks.iter().any(|datapack| {
            ...
            let already_installed = installed_datapacks
                .iter()
                .find(|&datapack| {
                    datapack.slug == current_datapack.slug
                        && current_datapack.world_name == datapack.world_name
        ...
        if !current_datapack_path.exists() || current_datapack_path.is_dir() {
            ...
            if let Some(url) = &current_datapack.url {
                info!(
                    "downloading datapack {} from {}",
                    &current_datapack.file_name, url
                );

                let retrieved_bytes = download_file(url, |a, b| {
                    progress.progress_update(ProgressUpdate::set_for_step(
                        ProgressUpdateSteps::DownloadDatapack,
                        get_progress(datapack_idx, a, b),
                        max,
                    ));
                })
                .await?;
                ...
                info!(
                    "Installed Datapack {} in world {}",
                    &current_datapack.file_name, &current_datapack.world_name
                );
            }
        } else {
            info!(
                "Datapack {} is already downloaded in world {}",
                &current_datapack.file_name, &current_datapack.world_name
            );
        }
...
pub async fn copy_custom_mods(
    data: &Path,
    manifest: &NoRiskLaunchManifest,
    progress: &impl ProgressReceiver,
) -> Result<()> {
    let mod_cache_path = data.join("custom_mods").join(format!(
        "{}-{}",
