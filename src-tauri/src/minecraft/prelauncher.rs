...
pub(crate) async fn launch<D: Send + Sync>(
    ...
    let data_path = LAUNCHER_DIRECTORY
        .data_dir()
        .join("gameDir")
        .join(&launch_manifest.build.branch)
        .join("nrc_cache");
    ...
    clear_mods(&data_directory, &launch_manifest)
        .await
        .or_else(|e| if multiple_instances { Ok(()) } else { Err(e) })?;
    ...
    let mut version =
        VersionProfile::download(&data_path.join("child_sub_system.json"), &manifest_url).await?;
    ...
    if let Some(inherited_version) = &version.inherits_from {
        ...
        let parent_version =
            VersionProfile::download(&data_path.join("parent_sub_system.json"), url).await?;
...
pub async fn retrieve_and_copy_mods(
    ...
    for (mod_idx, current_mod) in mods.iter().enumerate() {
        ...
        progress.progress_update(ProgressUpdate::set_label(format!(
            "translation.downloadingRecommendedMod&mod%{}",
            current_mod.name
        )));
        ...
        if !current_mod_path.exists() && current_mod.source.get_repository() != *"CUSTOM" {
        ...
        } else if !current_mod_path.exists() && current_mod.source.get_repository() == *"CUSTOM" {
            // Broken custom mod path -> ignore
            installed_mods.push(current_mod.clone());
            warn!(
                "Skipping Mod {:?} cuz it's a custom mod with a broken / non existing path!",
                current_mod
            );
            continue;
        }
