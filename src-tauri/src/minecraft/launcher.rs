...
pub struct LauncherData<D: Send + Sync> {
    ...
    pub(crate) on_progress:
        fn(&D, ProgressUpdate, Uuid, &Arc<Mutex<Vec<RunnerInstance>>>) -> Result<()>,
...
pub async fn launch<D: Send + Sync>(
    ...
    let java_bin = if let Some(path) = &launching_parameter.custom_java_path {
        ...
        match find_java_binary(&runtimes_folder, manifest.build.jre_version).await {
            ...
            Err(e) => {
                ...
                info!("Download JRE...");
                launcher_data_arc
                    .progress_update(ProgressUpdate::set_label("translation.downloadingJRE"));
        ...
        .and_then(|x| x.client.as_ref())
    {
        ...
        if requires_download {
            launcher_data_arc
                .progress_update(ProgressUpdate::set_label("translation.downloadingClient"));
    ...
    if natives_folder.exists() {
        ...
        fs::remove_dir_all(&natives_folder).await.or_else(|e| {
    ...
    launcher_data_arc.progress_update(ProgressUpdate::set_label(
        "translation.checkingMinecraftAssets",
    ));
    launcher_data_arc.progress_update(ProgressUpdate::set_for_step(
                    ...
                    Ok(downloaded) => {
                        ...
                        if downloaded {
                            ...
                            data_clone.progress_update(ProgressUpdate::set_label(format!(
                                "translation.downloadedMinecraftAsset&hash%{hash}"
                            )));
                        }
                    }
    ...
    if keep_local_assets {
        ...
        let json_data =
            ApiEndpoints::assets(manifest.build.branch.clone(), norisk_token, uuid).await;
        ...
        if !norisk_asset_objects_to_download.is_empty() {
            ...
            launcher_data_arc.progress_update(ProgressUpdate::set_label(
                "translation.checkingNoriskAssets",
            ));
            launcher_data_arc.progress_update(ProgressUpdate::set_for_step(
...
async fn verify_norisk_assets<D: Send + Sync>(
    ...
    launcher_data_arc.progress_update(ProgressUpdate::set_label(
        "translation.verifyingNoriskAssets",
    ));
    launcher_data_arc.progress_update(ProgressUpdate::set_for_step(
        ...
        if path.is_file() {
            ...
            if file_names.contains(&file_name.as_ref()) {
                ...
                launcher_data_arc.progress_update(ProgressUpdate::set_label(format!(
                    "translation.verifiedNoriskAsset&fileName%{file_name}"
                )));
            } else if let Err(err) = fs::remove_file(&path).await {
                info!("Failed to remove {}: {}", path.display(), err);
            } else {
                info!(
                    "Removed file {} since it was not found in the asset objects for this branch.",
                    path.display()
                );
            }
        }
