...
export function iconUrlForUuid(uuid: string | null | undefined): string | null {
  if (!uuid) return null;
  const name = ICON_NAME_BY_UUID[uuid];
  if (!name) return null;
  return `${ICONFONT_BASE}${name}_icons.png`;
}

export function creatorCodeIconUrl(code: string): string {
  return `${CREATORCODE_BASE}${code.toLowerCase()}.png`;
}
