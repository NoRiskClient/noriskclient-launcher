...
export function LogViewerCore({
  ...
  const commitSelectionIndices = useCallback(
  ...
  const copySelectedLogs = useCallback(() => {
    ...
    void writeText(text).catch((error) => {
      ...
      toast.error(
        t("logs.copy_failed", { defaultValue: "Failed to copy logs" }),
      );
    });
    return true;
  }, [showThreadPrefix, t]);
  ...
  useEffect(() => {
    ...
    const getEdgeScrollDelta = (clientY: number) => {
      ...
      if (clientY <= rect.top + edgeThreshold) {
        const intensity = Math.min(
          1,
          (rect.top + edgeThreshold - clientY) / edgeThreshold,
        );
        return -maxSpeed * (0.2 + intensity * 0.8);
      }
      return 0;
    ...
    const getVisibleBoundaryIndex = (
      direction: "up" | "down",
    ): number | null => {
      const rows = Array.from(
        container.querySelectorAll<HTMLElement>("[data-log-index]"),
      );
      if (rows.length === 0) return null;

      const boundaryRow = direction === "up" ? rows[0] : rows[rows.length - 1];
      const index = Number.parseInt(
        boundaryRow.getAttribute("data-log-index") ?? "",
        10,
      );
      return Number.isNaN(index) ? null : index;
    };
  ...
  const renderLogLine = (index: number, log: DisplayLogLine) => {
    ...
    return (
      <div
        ...
        style={
          isSelected ? { backgroundColor: `${accentColor.value}33` } : undefined
        }
      >
        {log.timestamp ? (
          <>
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 gap-2">
      ...
      <div
        ...
        onMouseDown={() =>
          logContainerRef.current?.focus({ preventScroll: true })
        }
