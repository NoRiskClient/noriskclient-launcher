<script>
    ...
    async function searchResourcePacks() {
        // if (searchterm === "") {
            ...
            // Wollen wir das für resourcePacks?
        ...
        await invoke("search_resourcepacks", {
            ...
            if (result.hits.length === 0) {
                ...
                resourcePacks = [...resourcePacks, ...result.hits];
            }
        }).catch((err) => {
            console.error(err);
        });
...
<div class="modrinth-wrapper">
    ...
    {:else if currentTabIndex === 1}
        <ModrinthSearchBar on:search={() => {}} bind:searchTerm={filterterm} placeHolder="Filter installed Resource Packs..."/>
