...
export function LocalContentTabV2<T extends LocalContentItem>({
  ...
  const noriskPackOptions = useMemo((): SelectOption[] => {
    ...
    return [
      { value: "", label: t("content.no_pack_selected_option") },
      ...options,
    ];
  ...
  const defaultOnAddContent = async () => {
    ...
    switch (currentContentType) {
      case "Mod":
        dialogFilters = [
          {
            name: t("content.filter.java_archives"),
      ...
      case "ShaderPack":
        dialogFilters = [
          {
            name: t("content.filter.shader_pack_archives"),
            extensions: ["zip", "zip.disabled"],
          },
        ];
        break;
      case "DataPack":
        dialogFilters = [
          {
            name: t("content.filter.data_pack_archives"),
            extensions: ["zip", "zip.disabled"],
          },
        ];
        break;
      default:
        toast.error(
          t("content.toast.import_not_configured", {
    ...
    try {
      ...
      const selectedPathsArray = await open({
        ...
        title: t("content.dialog.select_to_import", {
          itemType: itemTypeNamePlural,
      ...
      if (selectedPathsArray && selectedPathsArray.length > 0) {
        ...
        const toastId = toast.loading(
          t("content.toast.importing", {
            count: filePaths.length,
            itemType: itemTypeNamePlural.toLowerCase(),
          }),
        );
        try {
          ...
          toast.success(
            t("content.toast.import_success", {
              count: filePaths.length,
              itemType: itemTypeNamePlural.toLowerCase(),
            }),
            { id: toastId },
    ...
    const fetchVersionsForDropdown = async () => {
      if (openVersionDropdownId) {
        ...
        if (platform && projectId) {
          ...
          try {
            ...
            setVersionsError(
              error instanceof Error
                ? t("content.version_load_failed_details", {
          ...
          setVersionsError(
            currentItem
              ? t("content.version_history_unavailable", {
                  platform: getItemPlatformDisplayName(currentItem),
                })
        ...
        return (
          <span title={titleText} className="flex items-center">
            {versionText ? (
              <>
                <span>
                  {t("content.version_label")} {versionText}
                </span>
                          ...
                          {isLoadingVersions ? (
                            ...
                            <>
                              <div className="font-bold mb-2 text-sm tracking-wider flex-shrink-0">
                                Available Versions on{" "}
                                {getItemPlatformDisplayName(item)}:
                              </div>
                            ...
                            <div className="text-white/70 text-sm tracking-wider">
                              {t("content.no_compatible_versions", {
                                platform: getItemPlatformDisplayName(item),
                              })}
                            </div>
                          ) : (
                            <div className="text-white/70 text-sm tracking-wider">
      ...
      itemActions.push({
        ...
        label: isToggling
          ...
          : !item.is_disabled
            ? t("common.disable")
            : t("common.enable"),
        icon: !item.is_disabled
          ? "solar:close-circle-bold"
          : "solar:check-circle-bold",
        variant: !item.is_disabled ? "secondary" : "primary",
        tooltip: !item.is_disabled
          ? t("content.actions.disable_item")
          : t("content.actions.enable_item"),
        disabled: isToggling,
            ...
            {item.id && (
              ...
              >
                ...
                <span className="flex-1">
                  {(item.updates_enabled ?? true)
                    ? t("content.actions.disable_check")
            ...
            {selectedItemIds.size > 0 && (
              <ContentActionButtons
                actions={[
                  {
                    id: "batch-toggle",
                    label: isBatchToggling
                      ? t("content.actions.toggling")
                      : `${t("content.actions.toggle")} (${selectedItemIds.size})`,
                    icon: isBatchToggling
                      ? LOCAL_CONTENT_TAB_ICONS_TO_PRELOAD[11]
                      : "solar:refresh-bold",
                    variant: "text" as const,
                    disabled: isBatchToggling,
                    loading: isBatchToggling,
                    tooltip: t("content.actions.toggle_selected", {
                    ...
                    ? [
                        {
                          ...
                          tooltip: updatesToggleConfig.shouldEnable
                            ? t("content.actions.enable_checks_selected", {
                                count: updatesToggleConfig.actionCount,
                              })
                            : t("content.actions.disable_checks_selected", {
                                count: updatesToggleConfig.actionCount,
                              }),
                          onClick: () =>
                            handleBatchToggleSelectedUpdatesEnabled(
                              updatesToggleConfig.shouldEnable,
                            ),
                        },
                      ]
                    : []),
                  ...(contentType !== "NoRiskMod"
                    ? [
                        {
                          id: "batch-delete",
                          label: isBatchDeleting
                            ? t("content.actions.deleting")
                            : `${t("common.delete")} (${selectedItemIds.size})`,
                          icon: isBatchDeleting
                            ? LOCAL_CONTENT_TAB_ICONS_TO_PRELOAD[11]
                            : LOCAL_CONTENT_TAB_ICONS_TO_PRELOAD[6],
                          variant: "text" as const,
                          disabled: isBatchDeleting,
                          loading: isBatchDeleting,
                          tooltip: t("content.actions.delete_selected", {
                            count: selectedItemIds.size,
                          }),
                    ...
                    actions={[
                      {
                        id: "update-all",
                        label: isUpdatingAll
                          ? t("content.actions.updating_all")
                          : `${t("content.actions.update_all")} (${updatableContentCount})`,
                        icon: isUpdatingAll
                          ? LOCAL_CONTENT_TAB_ICONS_TO_PRELOAD[11]
                        ...
                        tooltip: t("content.actions.update_all_tooltip", {
                          count: updatableContentCount,
                        }),
                      ...
                      actions={[
                        {
                          ...
                          variant: "highlight" as const,
                          tooltip: t("content.actions.browse_tooltip", {
                            itemType: itemTypeNamePlural,
                          }),
                          onClick: () => {
                            if (profile && onBrowseContentRequest) {
                              ...
                              const browseContentType =
                                getBrowseTabContentType(contentType);
                              navigate(
                                `/profilesv2/${profile.id}/browse/${browseContentType}`,
                              );
                            }
                          },
                        },
      ...
      try {
        ...
        if (newName && typeof newName === "string") {
          ...
          toast.promise(clonePromise, {
            loading: t("content.toast.cloning", {
              name: profile.name,
              newName,
            }),
  ...
  return (
    <>
      ...
      <ConfirmDeleteDialog
        ...
        title={
          itemToDeleteForDialog
            ? t("content.delete_item_title", {
                name: getDisplayFileName(itemToDeleteForDialog),
              })
            : t("content.delete_selected_title", {
                itemType: itemTypeNamePlural,
              })
        }
      />
