...
function VersionItem({
  ...
  const [curseforgeChangelog, setCurseforgeChangelog] = useState<string | null>(
    null,
  );
          ...
          <div className="max-h-64 overflow-y-auto scrollbar-thin scrollbar-thumb-white/20 scrollbar-track-transparent">
            {isLoadingChangelog ? (
              <div className="flex items-center justify-center py-4">
                ...
                />
                <span className="ml-2 text-sm text-white/50 font-minecraft">
                  {t("modpack_versions.loading_changelog")}
                </span>
              </div>
            ) : displayChangelog ? (
              version.source === "CurseForge" ? (
                ...
                <div
                  ...
                  dangerouslySetInnerHTML={{
                    __html: sanitizeRichHtml(displayChangelog),
                  }}
                />
              ) : (
                ...
                <div className="prose prose-invert prose-sm max-w-none font-minecraft">
                  <ReactMarkdown
                    ...
                    components={{
                      ...
                      h3: ({ children }) => (
                        <h3 className="text-sm font-bold text-white mb-1 mt-2">
                          {children}
                        </h3>
                      ),
                      p: ({ children }) => (
                      ...
                      ul: ({ children }) => (
                        <ul className="list-disc list-inside text-sm text-white/90 mb-2 space-y-1 ml-4">
                          {children}
                        </ul>
                      ),
                      ol: ({ children }) => (
                        <ol className="list-decimal list-inside text-sm text-white/90 mb-2 space-y-1 ml-4">
                          {children}
                        </ol>
                      ),
                      li: ({ children }) => (
                      ...
                      pre: ({ children }) => (
                        <pre className="bg-black/30 p-2 rounded text-xs font-mono text-white/90 overflow-x-auto mb-2">
                          {children}
                        </pre>
                      ),
                      blockquote: ({ children }) => (
                        <blockquote className="border-l-2 border-accent pl-3 italic text-white/70 my-2">
                          {children}
                        </blockquote>
                      ),
                      a: ({ href, children }) => (
      ...
      ProfileService.getProfile(profileId)
        .then((profile) => {
          if (profile.modpack_info?.source) {
            return UnifiedService.getModpackVersions(
        ...
        .then((versions) => {
          ...
          console.log(
            "First version has changelog:",
            versions.all_versions[0]?.changelog,
          );
...
> **Note**: This version requires Java 17 or higher for optimal performance.`,
                ...
                version.changelog =
                  mockChangelogs[index % mockChangelogs.length];
              }
            });
          }
  ...
  const [selectedVersion, setSelectedVersion] = useState<UnifiedVersion | null>(
    null,
  );
  ...
  const handleSwitchVersion = async () => {
    ...
    if (profileId && selectedVersion.files.length > 0) {
      try {
        ...
        const primaryFile =
          selectedVersion.files.find((f) => f.primary) ||
        ...
        if (selectedVersion.source === "Modrinth") {
          ...
          throw new Error(
            `Unsupported modpack source: ${selectedVersion.source}`,
          );
        }
        ...
        const loadingToast = toast.loading(
          t("modpack_versions.toast.switching", {
            name: modpackName,
        ...
        toast.success(
          t("modpack_versions.toast.switch_success", {
            name: modpackName,
            version: selectedVersion.version_number,
          }),
        );
        ...
        toast.error(
          t("modpack_versions.toast.switch_failed", {
  ...
  return (
    <Modal
      ...
      titleIcon={
        <Icon icon="solar:archive-bold" className="w-6 h-6 text-blue-400" />
      }
      onClose={onClose}
      width="lg"
      ...
      footer={
        <div className="flex justify-end items-center gap-3">
          ...
          >
            {isSwitching
              ? t("modpack_versions.button.switching")
              : selectedVersion
                ? t("modpack_versions.button.switch_version")
                : t("modpack_versions.button.select_version")}
          </Button>
        </div>
      }
    >
      <div className="p-4">
        <div className="mb-4 text-sm text-white/70 font-minecraft">
          {isLoadingVersions ? (
            ...
            <>
              {versions.all_versions.length} version
              {versions.all_versions.length !== 1 ? "s" : ""} available
