...
export function look(r: CrashCheckResult): Look {
  if (r.status === "investigating")
    return {
      ...
      headlineKey: "crash_analysis.headline.investigating",
      btn: "warning",
    };
  ...
  if (r.classification === "nrc-own" && !r.known)
    return {
      ...
      headlineKey: "crash_analysis.headline.new_bug",
      btn: "warning",
    };
  if (r.source === "wiki")
    return {
      ...
      headlineKey: "crash_analysis.headline.known",
      btn: "success",
    };
  if (r.source === "auto")
    return {
      ...
      headlineKey: "crash_analysis.headline.suggested",
      btn: "info",
    };
  return {
    color: "#9ca3af",
    icon: "solar:inbox-line-bold",
    headlineKey: "crash_analysis.headline.logged",
    btn: "default",
  };
}
...
export const actionIcon = (a: CrashAction): string =>
  a.type === "resolve_conflict"
    ...
    : a.type === "enable_norisk_mod" || a.type === "enable_mod"
      ? "solar:bolt-circle-bold"
      : a.type === "disable_mod" || a.type === "disable_norisk_mod"
        ? "solar:power-bold"
        : a.type === "install_mod"
          ? "solar:download-minimalistic-bold"
          : "solar:refresh-bold";

const conflictMods = (a: CrashAction): string =>
  (a.targets?.length ? a.targets : [a.target]).join(" + ");
...
export function actionLabel(a: CrashAction, t: TFunction): string {
  if (a.type === "resolve_conflict")
    return t("crash_analysis.action.resolve_conflict", {
      mods: conflictMods(a),
    });
  if (a.type === "enable_norisk_mod")
    return t("crash_analysis.action.enable_norisk_mod", { mod: a.target });
  if (a.type === "disable_norisk_mod")
    return t("crash_analysis.action.disable_norisk_mod", { mod: a.target });
  if (a.type === "enable_mod")
    return t("crash_analysis.action.enable_mod", { mod: a.target });
  ...
  if (a.type === "install_mod")
    return t("crash_analysis.action.install_mod", { mod: a.target, version });
  if (a.type === "disable_mod")
    return t("crash_analysis.action.disable_mod", { mod: a.target });
...
export function statusMessageText(
  ...
  if (r.status === "investigating")
    return t("crash_analysis.status.investigating");
  if (r.classification === "nrc-own" && !r.known)
    return t("crash_analysis.status.new_nrc_bug");
...
export function summaryText(r: CrashCheckResult, t: TFunction): string | null {
  ...
  if (primary?.type === "resolve_conflict")
    return t("crash_analysis.summary.conflict", {
      mods: conflictMods(primary),
    });
  if (r.actions.length > 1 && r.actions.every((a) => a.type === "disable_mod"))
    return t("crash_analysis.summary.disable_one", {
      mods: r.actions.map((a) => a.target).join(" / "),
    });
  if (r.actions.length > 1)
    return t("crash_analysis.summary.multi", { count: r.actions.length });
  if (primary?.type === "update_loader" || primary?.type === "update_mod") {
    return primary.currentVersion
      ? t("crash_analysis.summary.loader_outdated", {
          mod: primary.target,
          current: primary.currentVersion,
          target: primary.targetVersion ?? "latest",
        })
      : t("crash_analysis.summary.loader_update", {
          mod: primary.target,
          target: primary.targetVersion ?? "latest",
        });
  }
  if (primary?.type === "install_mod")
    return t("crash_analysis.summary.install", {
      mod: primary.target,
      target: primary.targetVersion ?? "latest",
    });
  if (primary?.type === "disable_mod")
    return t("crash_analysis.summary.disable_mod", { mod: primary.target });
  if (primary?.type === "enable_norisk_mod")
    return t("crash_analysis.summary.enable_norisk_mod", {
      mod: primary.target,
    });
  if (primary?.type === "enable_mod")
    return t("crash_analysis.summary.enable_mod", { mod: primary.target });
  return r.summary;
}
