...
export async function getProcess(
  ...
  try {
    ...
    console.error(
      `[ProcessService] Failed to get process ${processId}:`,
      error,
    );
    return null;
  }
 ...
 */
export async function fetchCrashReport(
  profileId: string,
  processId?: string,
  processStartTime?: string,
): Promise<string | null> {
  console.debug(
    `[ProcessService] Fetching crash report for profile ${profileId}, process ${processId || "none"}, startTime ${processStartTime || "none"}`,
  );
  try {
 ...
 */
export async function checkCrashLog(
  payload: CrashlogDto,
): Promise<CrashCheckResult> {
