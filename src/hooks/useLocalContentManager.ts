...
import type {
  Profile,
  LocalContentItem as ProfileLocalContentItem,
  LoadItemsParams,
  CacheBehaviour,
} from "../types/profile";
import type {
...
import {
  getUpdateIdentifier,
  getContentPlatform,
} from "../utils/update-identifier-utils";
import { parseErrorMessage } from "../utils/error-utils";
import {
  contentCacheKey,
  useContentCacheStore,
} from "../store/content-cache-store";
...
function mapUiContentTypeToBackend(uiType: LocalContentType): NrContentType {
  switch (uiType) {
    ...
    case "NoRiskMod":
      return NrContentType.NoRiskMod;
    default:
      throw new Error(`Unsupported UI content type: ${uiType}`);
  }
}

// Helper to map backend ProfileLocalContentItem to frontend T (which extends LocalContentItem)
function mapBackendItemToFrontendType<T extends LocalContentItem>(
  rawItem: ProfileLocalContentItem,
): T {
...
function createUninstallPayload<T extends LocalContentItem>(
  ...
  if (uiContentType === "Mod") {
    if (item.source_type === "custom") {
      if (!item.path) {
        toast.error(
          i18n.t("content_manager.errors.custom_mod_missing_path_uninstall", {
      ...
      if (!item.sha1_hash) {
        toast.error(
          i18n.t("content_manager.errors.mod_missing_sha1_uninstall", {
            filename: item.filename,
          }),
        );
        return null;
      ...
      return {
        ...
        content_type: NrContentType.Mod,
      };
    }
  } else if (
    uiContentType === "ResourcePack" ||
    uiContentType === "ShaderPack" ||
    uiContentType === "DataPack"
  ) {
    if (!item.path) {
      toast.error(
        i18n.t("content_manager.errors.content_missing_path_uninstall", {
          type: uiContentType,
          filename: item.filename,
        }),
      );
    ...
    toast.error(
      i18n.t("content_manager.errors.norisk_uninstall_not_supported"),
    );
    console.error(
      "[useLocalContentManager] Attempted to create uninstall payload for NoRiskMod. This is generally not supported here.",
    );
    return null;
  }

  toast.error(
    i18n.t("content_manager.errors.unsupported_content_type_uninstall", {
      type: uiContentType,
    }),
  );
  return null;
...
function createTogglePayload<T extends LocalContentItem>(
  ...
  const payloadBase: Omit<
    ToggleContentPayload,
    "sha1_hash" | "file_path" | "norisk_mod_identifier"
  > = {
    profile_id: profileId,
  ...
  if (uiContentType === "NoRiskMod") {
    const noriskIdentifierFromItem = (item as ProfileLocalContentItem)
      .norisk_info; // Expect norisk_info from the item
    if (noriskIdentifierFromItem) {
      return {
        ...payloadBase,
        norisk_mod_identifier: noriskIdentifierFromItem,
      }; // Map to payload's norisk_mod_identifier
    } else {
      toast.error(
        i18n.t("content_manager.errors.norisk_missing_info_toggle", {
          filename: item.filename,
        }),
      );
      return null;
    ...
    if (item.source_type === "custom") {
      if (!item.path) {
        toast.error(
          i18n.t("content_manager.errors.custom_mod_missing_path_toggle", {
            filename: item.filename,
          }),
        );
        return null;
      ...
      if (item.sha1_hash) {
        const modPayload: ToggleContentPayload = {
          ...payloadBase,
          sha1_hash: item.sha1_hash,
        };
        if (item.path) {
        ...
        toast.error(
          i18n.t("content_manager.errors.mod_missing_identifiers_toggle", {
            filename: item.filename,
          }),
        );
        return null;
    ...
    if (item.path) {
      ...
      toast.error(
        i18n.t("content_manager.errors.path_missing_toggle", {
          type: uiContentType,
          filename: item.filename,
        }),
      );
...
export function useLocalContentManager<T extends LocalContentItem>({
  ...
  const initialEntry = profile?.id
    ? useContentCacheStore
        .getState()
        .getEntry<T>(contentCacheKey(profile.id, contentType))
    : undefined;

  const [items, setItems] = useState<T[]>(() => initialEntry?.items ?? []);
  const [isInitialLoadingState, setIsInitialLoadingState] = useState(
    () => !!profile?.id && !initialEntry,
  );
  ...
  const [isConfirmDeleteDialogOpen, setIsConfirmDeleteDialogOpen] =
    useState(false);
  const [itemToDeleteForDialog, setItemToDeleteForDialog] = useState<T | null>(
    null,
  );
  const [isBatchDeleteConfirmActive, setIsBatchDeleteConfirmActive] =
    useState(false);
  ...
  const getItemPlatform = useCallback(
    (item: T): "modrinth" | "curseforge" | "local" => {
  ...
  const getItemPlatformDisplayName = useCallback(
    (item: T): string => {
      const platform = getItemPlatform(item);
  ...
  const [contentUpdates, setContentUpdates] = useState<
    Record<string, UnifiedVersion>
  >(() => initialEntry?.contentUpdates ?? {});
  const [isCheckingUpdates, setIsCheckingUpdates] = useState(false);
  const [itemsBeingUpdated, setItemsBeingUpdated] = useState<Set<string>>(
    new Set(),
  );
  const [contentUpdateError, setContentUpdateError] = useState<string | null>(
    null,
  );
  const [isUpdatingAll, setIsUpdatingAll] = useState(false);

  const [isInitialLoadProcessComplete, setIsInitialLoadProcessComplete] =
    useState(false);
  ...
  const cacheKey = profile?.id
    ? contentCacheKey(profile.id, contentType)
    : null;
  ...
  const fetchBasicInfo = useCallback(
    async (
      behaviour: CacheBehaviour = "stale_while_revalidate",
    ): Promise<void> => {
      if (!profile?.id) {
        setItems([]);
        return;
      ...
      const cached =
        behaviour === "stale_while_revalidate"
      ...
      console.log(
        `[${contentType}] Loading local content...`,
      ...
      try {
        ...
        const fetchedBackendItems = (await getLocalContent(
          serviceParams,
        )) as ProfileLocalContentItem[];
        if (loadIdRef.current !== loadId) return; // superseded by a newer load

        const previousByPath = new Map(
          itemsRef.current.map((item) => [item.path, item]),
        );
        const mappedItemsToFrontend = fetchedBackendItems.map((item) =>
          mapBackendItemToFrontendType<T>(item),
        );
        const processedBasicItems = mappedItemsToFrontend.map((item) => {
          ...
          return {
            ...
            modrinth_info:
              item.modrinth_info ?? previous?.modrinth_info ?? null,
            curseforge_info:
              item.curseforge_info ?? previous?.curseforge_info ?? null,
          };
        });
        setItems(processedBasicItems as T[]);
        console.log(
          `[${contentType}] Items set (count: ${processedBasicItems.length})`,
          new Date().toISOString(),
        );
  ...
  const fetchData = useCallback(
    async (initialFetch = true): Promise<void> => {
      if (profile?.id) {
        useContentCacheStore
          .getState()
          .invalidate(contentCacheKey(profile.id, contentType));
      }
      if (initialFetch) {
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (
        ...
        const isMoreActionsButton = clickedElement.closest(
  ...
  useEffect(() => {
    const fetchModrinthIcons = async () => {
      ...
      const { modrinthIcons: cached, mergeModrinthIcons } =
        useIconCacheStore.getState();
      const projectIdsToFetch = items
        .filter((item) => {
          const platform = getItemPlatform(item);
          if (platform !== "modrinth" || !item.modrinth_info?.project_id)
            return false;
      ...
      if (uniqueProjectIds.length > 0) {
        try {
          const projectDetailsList = await ModrinthService.getProjectDetails(
            uniqueProjectIds,
            cacheBehaviourRef.current,
          );
          ...
          if (Array.isArray(projectDetailsList)) {
            ...
            console.warn(
              "[useLocalContentManager] ModrinthService.getProjectDetails did not return an array. Received:",
              projectDetailsList,
            );
          }
          ...
          console.error(
            "[useLocalContentManager] Failed to fetch Modrinth project details for icons:",
            err,
          );
        }
      }
  ...
  useEffect(() => {
    const fetchCurseForgeIcons = async () => {
      ...
      const { curseforgeIcons: cached, mergeCurseforgeIcons } =
        useIconCacheStore.getState();
      const projectIdsToFetch = items
        .filter((item) => {
          const platform = getItemPlatform(item);
          if (platform !== "curseforge" || !item.curseforge_info?.project_id)
            return false;
      ...
      if (uniqueProjectIds.length > 0) {
        try {
          const modsResponse = await CurseForgeService.getModsByIds(
            uniqueProjectIds,
            undefined,
            cacheBehaviourRef.current,
          );
          ...
          if (modsResponse && modsResponse.data) {
            ...
            console.warn(
              "[useLocalContentManager] CurseForgeService.getModsByIds did not return expected structure. Received:",
              modsResponse,
            );
          }
          ...
          console.error(
            "[useLocalContentManager] Failed to fetch CurseForge mod details for icons:",
            err,
          );
        }
      }
  ...
  useEffect(() => {
    const fetchLocalArchiveIcons = async () => {
      ...
      const { archiveIcons: cached, mergeArchiveIcons } =
        useIconCacheStore.getState();
      ...
      try {
        ...
        console.error(
          "[useLocalContentManager] Failed to fetch local archive icons:",
          err,
        );
      }
    };
  ...
  useEffect(() => {
    ...
    useContentCacheStore
      .getState()
      .setEntry(cacheKey, { items, contentUpdates });
  ...
  const areAllFilteredSelected = useMemo(() => {
    return (
      ...
      filteredItems.every((item) => selectedItemIds.has(item.filename))
    );
  ...
  const handleSelectAllToggle = useCallback(
    (isChecked: boolean) => {
      setSelectedItemIds((prev) => {
        const newSet = new Set(prev);
        if (isChecked)
          filteredItems.forEach((item) => newSet.add(item.filename));
  ...
  const handleToggleItemEnabled = useCallback(
    async (item: T) => {
      ...
      console.log(
        `[${contentType}] handleToggleItemEnabled: Item BEFORE toggle - Path: ${item.path}, Filename: ${item.filename}, Disabled: ${item.is_disabled}`,
      );
      ...
      const payload = createTogglePayload(
        item,
        profile.id,
        contentType,
        targetEnabledState,
      ...
      try {
        ...
        setItems((prevItems) =>
          prevItems.map((i) => {
            if (i.filename === item.filename) {
              ...
              console.log(
                `[${contentType}] handleToggleItemEnabled: Item AFTER toggle (in setItems) - Path: ${updatedItem.path}, Filename: ${updatedItem.filename}, Disabled: ${updatedItem.is_disabled}`,
              );
              return updatedItem;
            }
            return i;
        ...
        toast.error(
          i18n.t("content_manager.errors.toggle_failed", {
  ...
  const handleConfirmDeletion = useCallback(async () => {
    ...
    if (isBatchDeleteConfirmActive) {
        ...
        if (item) {
          ...
          if (payload) {
            try {
              ...
              errors.push(
                i18n.t("content_manager.errors.delete_failed", {
                  name: getDisplayFileName(item),
                  error: errorDetail,
                }),
              );
            ...
            errors.push(
              `Could not create uninstall payload for ${getDisplayFileName(item)}.`,
            );
          }
        } else {
      ...
      if (errors.length > 0)
        toast.error(
          i18n.t("content_manager.errors.batch_delete_failed", {
            errors: errors.join("; "),
          }),
        );
      if (successfulOperations > 0)
        toast.success(
          i18n.t("content_manager.success.batch_deleted", {
      ...
      const payload = createUninstallPayload(
      ...
      if (payload) {
        try {
          await uninstallContentFromProfile(payload);
          toast.success(
            i18n.t("content_manager.success.deleted", {
          ...
          setItems((prevItems) =>
            prevItems.filter(
              (i) => i.filename !== itemToDeleteForDialog.filename,
            ),
          );
    ...
    if (successfulOperations > 0 || errors.length > 0) {
      if (isBatchDeleteConfirmActive || errors.length > 0) {
        // Refresh if batch or single error
        await fetchData(true); // Full refresh
      }
      if (onRefreshRequiredRef.current) onRefreshRequiredRef.current();
    }
    if (errors.length > 0 && !isBatchDeleteConfirmActive)
  ...
  const handleBatchToggleSelected = useCallback(async () => {
        ...
        if (payload) {
          try {
            ...
            setItems((prev) =>
              prev.map((i) =>
                i.filename === itemId
                  ? { ...i, is_disabled: !targetEnabledStateForItem }
                  : i,
              ),
            );
            successfulOperations++;
            ...
            const errorDetail = parseErrorMessage(err);
            errors.push(
              `Failed to toggle ${getDisplayFileName(item)}: ${errorDetail}`,
            );
          }
        } else {
          // Error already toasted by createTogglePayload if it returned null
          errors.push(
            `Could not create toggle payload for ${getDisplayFileName(item)}.`,
          );
        }
      } else {
    ...
    if (errors.length > 0)
      toast.error(
        i18n.t("content_manager.errors.batch_toggle_failed", {
          errors: errors.join("; "),
        }),
      );
    if (successfulOperations > 0) {
      toast.success(
        i18n.t("content_manager.success.batch_toggled", {
          count: successfulOperations,
        }),
      );
      if (onRefreshRequiredRef.current) onRefreshRequiredRef.current();
    ...
    try {
      ...
      try {
        ...
        const errorMsg =
          openError?.message ||
          revealError?.message ||
          "Failed to open item location.";
        toast.error(
          i18n.t("content_manager.errors.failed_open_location", {
            error: errorMsg,
          }),
        );
      }
  ...
  const checkForContentUpdates = useCallback(
    async (currentProfile = profile, currentItems = items) => {
      ...
      const itemsWithIdentifiers = currentItems.filter(
        (item) =>
      ...
      for (const item of itemsWithIdentifiers) {
        ...
        const platform =
          getContentPlatform(item) === "CurseForge"
      ...
      for (const { identifier, platform, item } of itemIdentifiers) {
        ...
        if (
          ...
          item.curseforge_info?.fingerprint
        ) {
      ...
      try {
        const request: UnifiedUpdateCheckRequest = {
          ...
          algorithm: "sha1",
          loaders:
            (contentType === "Mod" || contentType === "NoRiskMod") &&
          ...
          hash_fingerprints:
            Object.keys(hashFingerprints).length > 0
              ? hashFingerprints
              : undefined,
        };

        // Verwende den UnifiedService
        const updates: UnifiedUpdateCheckResponse =
          await UnifiedService.checkModUpdates(
            request,
            cacheBehaviourRef.current,
          );
        ...
        Object.entries(updates.updates)
          .slice(0, 3)
          .forEach(([key, update]) => {
            console.log(
              `  Key: "${key}" -> ${update.version_number} (${update.source})`,
            );
          });
        ...
        console.log(
          "Items by identifier keys:",
          Array.from(itemsByIdentifier.keys()),
        );
        console.log("Update keys to process:", Object.keys(updates.updates));

        for (const [identifier, unifiedVersion] of Object.entries(
          updates.updates,
        )) {
          const item = itemsByIdentifier.get(identifier);
          console.log(
            `Processing update key "${identifier}": item found = ${!!item}`,
          );
          if (item) {
            console.log(
              `  Item: ${item.filename}, platform: ${item.platform}, has curseforge_info: ${!!item.curseforge_info}, fingerprint: ${item.curseforge_info?.fingerprint}, sha1_hash: ${item.sha1_hash}`,
            );
          } else {
            // Try to find item with different identifier formats
            console.log(`  Looking for item with fingerprint ${identifier}...`);
            const foundByFingerprint = items.find(
              (i) => i.curseforge_info?.fingerprint?.toString() === identifier,
            );
            if (foundByFingerprint) {
              console.log(
                `  Found by fingerprint: ${foundByFingerprint.filename}`,
              );
            } else {
          ...
          if (item && unifiedVersion && unifiedVersion.id) {
            ...
            const currentVersionId =
              ...
              item.id;
            console.log(
              `Comparing versions for ${item.filename}: current=${currentVersionId} (${typeof currentVersionId}), update=${unifiedVersion.id} (${typeof unifiedVersion.id})`,
            );
            if (currentVersionId !== unifiedVersion.id) {
              console.log(
                `Found update for ${item.filename}: ${currentVersionId} -> ${unifiedVersion.id}`,
              );
              // Verwende UnifiedVersion direkt ohne Konvertierung
              filteredUpdates[identifier] = unifiedVersion as UnifiedVersion;
            } else {
              console.log(
                `No update needed for ${item.filename}: versions match (${currentVersionId})`,
              );
            }
          } else {
            console.log(
              `Skipping update for key "${identifier}": item=${!!item}, unifiedVersion=${!!unifiedVersion}, unifiedVersion.id=${unifiedVersion?.id}`,
            );
          }
        }
        ...
        setContentUpdateError(
          `Error checking for ${contentType} updates: ${errorMsg}`,
        );
        setContentUpdates({});
      } finally {
  ...
  const performContentVersionSwitch = useCallback(
    async (
      item: T,
      newVersion: UnifiedVersion,
      options: {
      ...
      const primaryFile =
        newVersion.files.find((f) => f.primary) || newVersion.files[0];
      if (!primaryFile)
        throw new Error(
          `${options.isUpdateOperation ? "Updated" : "Switched"} version details are missing a primary file.`,
        );
      ...
      setItems((prevItems) =>
        prevItems.map((i) => (i.filename === item.filename ? updatedItem : i)),
      );
  ...
  const handleUpdateContentItem = useCallback(
    async (
      item: T,
      updateVersion: UnifiedVersion,
      suppressOwnToast: boolean = false,
    ) => {
      // 1. Initial checks
      if (!profile) {
      ...
      if (
        contentType === "Mod" &&
        item.modpack_origin !== null &&
        item.modpack_origin !== undefined &&
        item.updates_enabled !== true
      ) {
        toast.error(
          i18n.t("content_manager.errors.modpack_mod_update_blocked", {
            name: getDisplayFileName(item),
          }),
        );
        return;
      ...
      if (
        ...
        toast.error(
          i18n.t("content_manager.errors.item_path_missing_update", {
            name: getDisplayFileName(item),
          }),
        );
        return;
      ...
      if (
        ...
        toast.error(
          i18n.t("content_manager.errors.mod_not_modrinth", {
            name: getDisplayFileName(item),
          }),
        );
        return;
      ...
      if (
        ...
        toast.error(
          i18n.t("content_manager.errors.mod_not_curseforge", {
            name: getDisplayFileName(item),
          }),
        );
        return;
      ...
      console.log(
        `[${contentType}] Updating ${getDisplayFileName(item)} to version ${updateVersion.version_number} (${platform})`,
      );
      ...
      try {
        if (suppressOwnToast) {
          ...
          await toast.promise(
            promiseAction(),
            {
              loading: i18n.t("content_manager.loading.updating", {
                name: getDisplayFileName(item),
                version: updateVersion.version_number,
              }),
              success: i18n.t("content_manager.success.updated", {
                name: getDisplayFileName(item),
                version: updateVersion.version_number,
              }),
              error: (err: any) => {
                console.error(
                  `Failed to update ${contentType} for ${getDisplayFileName(item)} (${platform}):`,
                  err,
                );
                const errorMsg =
                  err?.message ||
                ...
                return i18n.t("content_manager.errors.update_failed", {
                  name: getDisplayFileName(item),
                  error: errorMsg,
                });
              },
        ...
        console.error(
          `Outer catch during update process for ${getDisplayFileName(item)}:`,
          err,
        );
    ...
    [
      ...
      contentType,
      getDisplayFileName,
      setItemsBeingUpdated,
      setContentUpdateError,
      setContentUpdates,
      performContentVersionSwitch,
    ],
  );
  ...
  const handleUpdateAllAvailableContent = useCallback(async () => {
    ...
    for (const item of items) {
      ...
      if (updateIdentifier && contentUpdates[updateIdentifier]) {
        ...
        const hasUpdatesEnabled =
          contentType === "Mod" && item.updates_enabled !== false;

        if (hasUpdatesEnabled) {
          itemsToUpdateWithDetails.push({
            item,
            version: contentUpdates[updateIdentifier],
          });
        } else {
          console.log(
            `Skipping ${getDisplayFileName(item)} from bulk update - UpdatesDisabled: ${!hasUpdatesEnabled}`,
          );
        }
      }
    }
    ...
    const toastId = toast.loading(
      i18n.t("content_manager.loading.updating_progress", {
        current: 0,
        total: totalCount,
        type: contentType,
      }),
    ...
    for (const { item, version } of itemsToUpdateWithDetails) {
      try {
        ...
        const errorMsg = parseErrorMessage(err);
        toast.error(
          i18n.t("content_manager.errors.update_failed", {
            name: getDisplayFileName(item),
            error: errorMsg,
          }),
        );
    ...
    if (failedCount > 0) {
      if (totalCount > 1) {
        const message = i18n.t(
          "content_manager.update_result.finished_partial",
      ...
      toast.success(
        i18n.t("content_manager.success.updated_all", {
          count: succeededCount,
          type: contentType,
        }),
        { id: toastId, duration: 700 },
      );
    } else {
      toast.dismiss(toastId);
    }
  ...
  const handleSwitchContentVersion = useCallback(
    async (item: T, newVersion: UnifiedVersion) => {
      if (!profile) {
      ...
      await toast.promise(
        promiseAction(),
        {
          loading: i18n.t("content_manager.loading.switching", {
            version: newVersion.name,
          }),
          success: i18n.t("content_manager.success.switched", {
            name: getDisplayFileName(item),
            version: newVersion.name,
          }),
          error: (err) =>
            i18n.t("content_manager.errors.switch_failed", {
  ...
  useEffect(() => {
    ...
    if (profile?.id && items.length > 0 && isInitialLoadProcessComplete) {
      console.log(
        `[${contentType}] Initial load process complete. Triggering checkForContentUpdates.`,
      );
      checkForContentUpdates();
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const handleToggleItemUpdatesEnabled = useCallback(
    async (item: T) => {
      if (!profile) {
        toast.error(
          i18n.t("content_manager.errors.profile_missing_toggle_updates"),
        );
        return;
      }
      ...
      if (!item.id) {
        toast.error(
          i18n.t("content_manager.errors.no_valid_id_toggle_updates", {
            name: getDisplayFileName(item),
          }),
        );
        return;
      ...
      await toast.promise(
        promiseAction(),
        {
          loading: i18n.t(
            newUpdatesEnabled
              ? "content_manager.loading.enabling_update_checks"
              : "content_manager.loading.disabling_update_checks",
            { name: getDisplayFileName(item) },
          ),
          success: i18n.t(
            newUpdatesEnabled
              ? "content_manager.success.toggle_updates_enabled"
              : "content_manager.success.toggle_updates_disabled",
            { name: getDisplayFileName(item) },
          ),
          error: (err) =>
            i18n.t("content_manager.errors.toggle_update_checks_failed", {
              error: err.message?.toString() || "Unknown error",
            }),
        },
        {
          success: {
  ...
  const handleBatchToggleSelectedUpdatesEnabled = useCallback(
    async (updatesEnabled: boolean) => {
      if (!profile) {
        toast.error(
          i18n.t("content_manager.errors.profile_missing_toggle_updates"),
        );
      ...
      const itemLabel = i18n.t(
        selectedItems.length === 1
          ? "content_manager.update_result.item"
          : "content_manager.update_result.items",
      );
      ...
      const promiseAction = async () => {
        ...
        if (validItems.length === 0) {
          throw new Error(
            "No valid items found with IDs for update check toggle",
          );
        }
        ...
        setItems((prevItems) =>
          prevItems.map((prevItem) =>
            validItems.some(
              (validItem) => validItem.filename === prevItem.filename,
            )
      ...
      await toast
        .promise(
          promiseAction(),
          {
            loading: i18n.t(
              updatesEnabled
                ? "content_manager.loading.enabling_update_checks_batch"
                : "content_manager.loading.disabling_update_checks_batch",
              { count: selectedItems.length, itemLabel },
            ),
            success: i18n.t(
              updatesEnabled
                ? "content_manager.success.batch_toggle_updates_enabled"
                : "content_manager.success.batch_toggle_updates_disabled",
              { count: selectedItems.length, itemLabel },
            ),
            error: (err) =>
              i18n.t("content_manager.errors.toggle_update_checks_failed", {
        ...
        .catch((err) => {
          ...
          if (err.message && err.message.includes("No valid items found")) {
            toast.error(
              i18n.t("content_manager.errors.no_valid_ids_toggle_updates"),
            );
          }
        });
    },
